...
- consume Agency Agent snapshot v1.4 capability summaries;
- normalize only read / effective write / effective commands / policy-valid / tool-count signals;
- persistent AgentProfile cards show one compact FR/EN capability badge;
- raw `tools_allowed`, `allowed_commands`, `allowed_changes`, repository paths and credentials are discarded;
- task execution authority remains exclusively in Agency Agent Operator;
- no policy-edit controls are added to Agent World.
...
Agent World Quality #98                                         ✅
real Windows project-zone navigation smoke                      ✅
```

Real Windows evidence confirmed that the **SilverKen Platform** project sidebar exposes **Ouvrir le projet dans Operator**, and the button opens the authoritative Operator directly on **Vue d’ensemble** without task context. Task Evidence & Trace and AgentProfile navigation remain unchanged.
